...
var BAD_CONNECTION = "ECONNREFUSED"
}
...
LoadTestCluster.prototype.run = function(specs) {
    ...
    self.cluster.on('slaveError', function(slave, err) {
        console.log('There is an error on the slave at: ' + slave['id'])
        if(err === null){
            console.log('Slave has failed to send its state in the last 4 intervals')
        }
        else if(err['code'] === BAD_CONNECTION){
            console.log('Slave has quit and is no longer responding')
        }
    });
};
